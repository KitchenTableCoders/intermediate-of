...
void testApp::draw(){
    ...
    float w = ofGetWidth()/(float)COLUMNS; // width of a single cell on the screen

    float h = ofGetHeight()/(float)ROWS;   // height of a single cell on screen
    float sw = vidGrabber.getWidth()/(float)COLUMNS; // width of a single cell from camera
    float sh = vidGrabber.getHeight()/(float)ROWS;     // height of a single cell from camera
    int i=0;
